...
.NOTES
    ...
    Updated : 8th January, 2026 - Added logic to handle installing with existing SQL Express instances.
...
function Get-SqlExpressInstances {
    <#
    .SYNOPSIS
        Enumerates local SQL Server Express instances (including multiple).
    .OUTPUTS
        PSCustomObject[] with Instance, Edition, Version
    #>
    [CmdletBinding()]
    param()
    ...
    foreach ($path in $basePaths) {
        ...
        foreach ($prop in $instanceMap.PSObject.Properties) {
            ...
            if ($setup -and $setup.Edition -and ($setup.Edition -like "*Express*")) {
                $expressInstances += [PSCustomObject]@{
                    Instance = $instanceName
                    Edition  = $setup.Edition
                    Version  = $setup.Version
    ...
    # De-dupe by instance name
    $expressInstances | Sort-Object Instance -Unique
}
...
function Select-SqlInstanceFromList {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [object[]]$Instances,

        [string]$Prompt = "Select the SQL instance to use"
    )

    # Exactly one instance found: confirm before continuing
    if ($Instances.Count -eq 1) {
        $one = $Instances[0]

        Info "One SQL Express instance was detected:"
        Write-Host ("  Instance: {0}" -f $one.Instance) -ForegroundColor Gray
        Write-Host ("  Edition : {0}" -f $one.Edition) -ForegroundColor Gray
        Write-Host ("  Version : {0}" -f $one.Version) -ForegroundColor Gray
        Write-Host ""

        if (-not (Confirm-YesNo -Message "Do you want to use this SQL Express instance")) {
            throw "User declined to use the detected SQL Express instance."
        }

        Ok "User confirmed SQL Express instance."
        return $one
    }

    # Multiple instances: present menu
    Info $Prompt
    for ($i = 0; $i -lt $Instances.Count; $i++) {
        $inst = $Instances[$i]

        $editionText = $inst.Edition
        if ([string]::IsNullOrWhiteSpace($editionText)) { $editionText = "Unknown" }

        $versionText = $inst.Version
        if ([string]::IsNullOrWhiteSpace($versionText)) { $versionText = "Unknown" }

        Write-Host ("[{0}] {1} ({2}) Version: {3}" -f ($i + 1), $inst.Instance, $editionText, $versionText) -ForegroundColor Gray
    }

    while ($true) {
        $choice = Read-Host ("Enter selection [1-{0}]" -f $Instances.Count)
...
try{
   # Validate Service Credentials
    ...
    # -----------------------------------
    $UseSqlExpress     = $true
    $InstallSqlExpress = $true   # assume install until we detect otherwise
    ...
    if (Test-SqlServerInstalled) {
        ...
        # Keep your existing full SQL prompt loop here (unchanged)
    }
    else {
        $express = @(Get-SqlExpressInstances)

        if ($express -and $express.Count -gt 0) {
            Ok "SQL Server Express detected. Will use an existing Express instance (no reinstall)."

            $selected = Select-SqlInstanceFromList -Instances $express -Prompt "Select the SQL Express instance to use"

            Warn "IMPORTANT: The account running this installer must have permission to create databases on this instance."
            Warn "If you do not have sysadmin/dbcreator (or equivalent), the MSI may fail creating the database."

            $SqlAddress  = "localhost"
            $SqlPort     = "1433"
            $SqlInstance = $selected.Instance
            $SqlDatabase = "PowerSyncProDB"

            $svc = Test-SqlInstanceService -InstanceName $SqlInstance
            if (-not $svc) {
                throw ("Selected SQL Express instance '{0}' is not valid on this system." -f $SqlInstance)
            }
            ...
            Info "No SQL Server found. SQL Express will be installed and used."
            $UseSqlExpress     = $true
    ...
    if ($InstallSqlExpress) {
        Info "Installing PowerSyncPro using NEW SQL Express configuration..."
        ...
        Info ("Installing PowerSyncPro using EXISTING SQL Express instance '{0}'..." -f $SqlInstance)
        if ($UseServiceAccount -eq $true) {
